...
g1 <- ggplot(NULL) +
  ...
  geom_point(
    aes(4, 0.1), size = 3, shape = 21, fill = "blue", stroke = 1, alpha = 0.5
  ) +
  geom_segment(
  ...
  geom_point(
    aes(4, 0.3), size = 3, shape = 21, fill = "red", stroke = 1, alpha = 0.5
  ) +
  geom_point(
    aes(5, 0.3), size = 3, shape = 21, fill = "blue", stroke = 1, alpha = 0.5
  ) +
  geom_segment(
...
g2 <- ggplot(NULL) +
  ...
  geom_segment(
    aes(0 + epsilon, 0.1, xend = 4 - epsilon, yend = 0.1),
    arrow = arrow(length = unit(0.2, "inches")), lwd = 1, lty = 1
  ) +
  geom_segment(
    aes(4 - epsilon, 0.1, xend = 0 + epsilon, yend = 0.1),
    arrow = arrow(length = unit(0.2, "inches")), lwd = 1, lty = 1
  ) +
  annotate(
    "text", x = 2, y = 0.1, label = c("Possible exposure dates"), vjust = -1
  ) +
  scale_x_continuous("Day",
    limits = c(-0.2, 5.5), expand = c(0, 0),
    breaks = 0:5,
...
tikz(
  file = "figure_latent_guide.tex", width = 10, height = 4, standAlone = TRUE
)
grid.arrange(g1, g2, nrow = 1)
dev.off()
tools::texi2dvi(
  "figure_latent_guide.tex", pdf = TRUE, clean = TRUE
)
